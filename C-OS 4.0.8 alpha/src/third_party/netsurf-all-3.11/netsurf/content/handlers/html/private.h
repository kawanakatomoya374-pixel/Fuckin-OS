...
typedef struct html_content {
	...
	/** True only while a deferred QuickJS/libdom mutation rebox is active. */
	bool cos_dom_mutation_rebuild;
	/** Box tree, or NULL. */
	struct box *layout;
...
/**
 * Rebuild the layout box tree from an already-live DOM after script mutation.
 * This must be invoked only when no DOM-to-box conversion is active.
 *
 * \param htmlc HTML content with a parsed, live document
 * \return true if a replacement conversion was queued
 */
