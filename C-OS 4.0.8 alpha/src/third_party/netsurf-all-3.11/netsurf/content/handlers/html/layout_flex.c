		...
		html_content *content)
{
	...
	NSLOG(flex, INFO, "C-OS flex layout box=%p direction=%s display=%u items=%zu available_width=%i width=%i",
			flex, ctx->horizontal ? "row" : "column",
			(unsigned int)css_computed_display_static(flex->style),
			ctx->item.count, available_width, flex->width);
