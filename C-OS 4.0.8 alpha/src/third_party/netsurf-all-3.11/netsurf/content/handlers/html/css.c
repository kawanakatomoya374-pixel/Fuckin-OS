...
#ifdef COS_KERNEL
	/* Large dynamically-injected style blocks from script-heavy sites such
	 * as Google can monopolise the small kernel CSS arena.  Native page CSS
	 * remains supported; only an oversized DOM-generated block is declined.
	 * This prevents a pathological CSS conversion loop without affecting
	 * Wikipedia's normal stylesheet and image layout. */
	if (dom_string_byte_length(style) > 32768) {
		NSLOG(netsurf, INFO, "Skipping oversized dynamic stylesheet (%zu bytes)",
		      dom_string_byte_length(style));
...
#ifdef COS_KERNEL
		/* Parse each source stylesheet once. This covers ordinary <style>
		 * blocks during document construction, including the user's page, while
		 * retaining the existing conservative policy for later script-driven
		 * replacement of a CSS arena already in use. */
		if (s->sheet != NULL) {
			s->modified = false;
			return true;
		}
...
#ifdef COS_KERNEL
	/* The C-OS GUI scheduler first runs after the parser has completed. If an
	 * initial <style> waits for that deferred callback, html_finish_conversion
	 * builds the one-shot NetSurf box tree with only UA CSS, then permanently
	 * ignores the author sheet when it becomes ready. Register initial author
	 * CSS synchronously while parsing, so base.active keeps conversion pending
	 * until the stylesheet is available. Later DOM mutations remain deferred. */
	if (c->select_ctx == NULL) {
		return html_css_process_modified_style(c, s);
	}
#endif
...
html_css_new_selection_context(html_content *c, css_select_ctx **ret_select_ctx)
{
	...
	for (i = STYLESHEET_BASE; i != c->stylesheet_count; i++) {
		...
		if (sheet != NULL) {
			NSLOG(netsurf, INFO,
			      "C-OS CSS selection append slot=%u origin=%u", i, origin);
			...
			if (css_ret != CSS_OK) {
				NSLOG(netsurf, ERROR,
				      "C-OS CSS selection append failed slot=%u error=%d", i, css_ret);
