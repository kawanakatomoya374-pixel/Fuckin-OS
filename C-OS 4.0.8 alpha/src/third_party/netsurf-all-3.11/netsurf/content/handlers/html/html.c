...
#include "../../../../../quickjs/quickjs_port.h"
...
/* Defined in the teardown section below; mutation rebox is implemented near
 * conversion completion and needs the same layout-owner cleanup routine. */
		...
		bool bubbles, bool cancelable)
{
	...
	/* libdom may report a text descendant as the target of a pointer hit.
	 * Feed the same trusted event through the C-OS QuickJS bridge, which walks
	 * ancestors and runs listeners registered on the owning element. */
	if (target != NULL && type != NULL) {
		char event_name[64];
		size_t event_len = dom_string_length(type);
		if (event_len >= sizeof(event_name)) event_len = sizeof(event_name) - 1;
		memcpy(event_name, dom_string_data(type), event_len);
		event_name[event_len] = '\0';
		(void)cos_js_dispatch_bound_dom_event(target, event_name);
	}
	dom_event_unref(evt);
	return result;
}
...
static void html_box_convert_done(html_content *c, bool success)
{
	...
	/* An already converted page retains its live libdom document but its
	 * hubbub parser was destroyed during initial conversion. A script-driven
	 * rebox must therefore not repeat the initial READY/DONE transition or
	 * destroy c->parser a second time (NULL here caused the observed #GP).
	 * The browser window remains the existing content user; notify only after
	 * the new boxes and image maps are ready. */
	if (c->cos_dom_mutation_rebuild) {
		c->cos_dom_mutation_rebuild = false;
		dom_node_unref(html);
		cos_netsurf_browser_dom_rebuild_complete();
		return;
	}

	/* Initial document conversion owns the parser-to-live-DOM transition. */
	if (c->parser != NULL) {
...
/* Clear live-DOM references to an obsolete box tree before its talloc context
 * is released.  Converted NetSurf documents normally keep these mappings for
 * their complete lifetime, but a QuickJS-driven rebox preserves the DOM. */
static void html_clear_box_node_data_subtree(dom_node *node)
{
	dom_node *child = NULL;
	dom_node *next = NULL;
	void *old_box = NULL;
	...
	if (node == NULL) return;
	(void)dom_node_set_user_data(node, corestring_dom___ns_key_box_node_data,
			NULL, NULL, &old_box);
	...
	while (child != NULL) {
		...
		html_clear_box_node_data_subtree(child);
...
/* Rebuild the visual box tree from the current live document. NetSurf 3.11
 * normally treats converted documents as immutable: html_reformat() only
 * reflows existing boxes. A real QuickJS/libdom mutation therefore requires a
 * fresh DOM-to-box conversion, deferred by the frontend until script dispatch
 * has returned. */
bool html_rebuild_layout_after_dom_mutation(html_content *htmlc)
{
	struct form *form;
	...
	if (htmlc == NULL || htmlc->document == NULL || htmlc->aborted ||
			htmlc->box_conversion_context != NULL) {
		return false;
	}

	/* Both libcss and the box constructor cache per-node pointers.  The DOM
	 * survives a rebox, so clear those pointers before the old forms and bctx
	 * release their objects.  Do not call box_free(): every box already has a
	 * talloc destructor and manual destruction double-frees its CSS results. */
	html_clear_box_node_data_subtree((dom_node *)htmlc->document);
	html_css_clear_node_data((dom_node *)htmlc->document);
	if (htmlc->select_ctx != NULL) {
		css_select_ctx_destroy(htmlc->select_ctx);
		htmlc->select_ctx = NULL;
	}
	if (html_css_new_selection_context(htmlc, &htmlc->select_ctx) != NSERROR_OK) {
		NSLOG(netsurf, ERROR, "DOM mutation rebox could not recreate CSS context");
		return false;
	}

	html_object_free_objects(htmlc);
	imagemap_destroy(htmlc);
	for (form = htmlc->forms; form != NULL; form = next_form) {
		next_form = form->prev;
		form_free(form);
	}
	htmlc->forms = NULL;
	...
	/* Discover form controls again so elements inserted by innerHTML obtain
	 * real gadgets rather than stale controls belonging to removed nodes. */
	htmlc->forms = html_forms_get_forms(htmlc->encoding,
			(dom_html_document *)htmlc->document);

	exc = dom_document_get_document_element(htmlc->document, (void *)&html);
	if (exc != DOM_NO_ERR || html == NULL) {
		NSLOG(netsurf, ERROR, "DOM mutation rebox has no document element");
		return false;
	}

	html_get_dimensions(htmlc);
	htmlc->cos_dom_mutation_rebuild = true;
	error = dom_to_box(html, htmlc, html_box_convert_done,
				   &htmlc->box_conversion_context);
	dom_node_unref(html);
	if (error != NSERROR_OK) {
		htmlc->cos_dom_mutation_rebuild = false;
		NSLOG(netsurf, ERROR, "DOM mutation rebox scheduling failed: %d", error);
		return false;
	}

	NSLOG(netsurf, INFO, "Queued DOM mutation box rebuild (content %p)", htmlc);
	return true;
}
...
#else
	/* C-OS deliberately ignores dynamic inline style mutation because its
	 * QuickJS port does not expose a mutable libdom.  Do not let a page leave
	 * a synthetic modified flag set forever and block HTML box conversion. */
	(void)i;
#endif
...
html_begin_conversion(html_content *htmlc)
{
	 ...
	 * the Document. The compact QuickJS backend routes the safe page-level
	 * portion to document/window listeners; it does not expose a raw native
	 * node pointer to script. This point is after deferred scripts, matching
	 * the HTML lifecycle ordering needed by framework bootstrap code. */
	if (htmlc->jsthread != NULL) {
		(void)js_fire_event(htmlc->jsthread, "DOMContentLoaded",
				    htmlc->document, (struct dom_node *)htmlc->document);
	}
...
#ifndef COS_KERNEL
/* Needs real fopen/fseek/ftell/fread/fclose to read the dropped
 * file's contents from disk. Drag-and-drop itself needs a working
 * GUI frontend (see PORTING_NOTES.md item 2) before this could ever
 * be reached anyway, and when that exists, this should read via
 * C-OS's own fs_api rather than fopen() - so left unported rather
 * than given a fake FILE* for now. */
