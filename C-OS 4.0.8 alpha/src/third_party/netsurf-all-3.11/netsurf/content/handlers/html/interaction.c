			      ...
			      bool trace_click)
{
	...
	do {
		...
		if (trace_click) {
			dom_string *trace_name = NULL;
			if (box->node != NULL &&
			    dom_node_get_node_name(box->node, &trace_name) == DOM_NO_ERR) {
				NSLOG(netsurf, INFO,
				      "click-hit box=%p type=%d pos=(%d,%d) size=(%d,%d) node=%p <%s> gadget=%p",
				      box, box->type, box_x, box_y, box->width, box->height,
				      box->node, dom_string_data(trace_name), box->gadget);
				dom_string_unref(trace_name);
			} else {
				NSLOG(netsurf, INFO,
				      "click-hit box=%p type=%d pos=(%d,%d) size=(%d,%d) node=%p gadget=%p",
				      box, box->type, box_x, box_y, box->width, box->height,
				      box->node, box->gadget);
			}
		}
