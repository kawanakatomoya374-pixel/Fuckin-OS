		...
		dom_string *src)
{
	...
	/* Wikimedia's portal scripts are optional for the static landing page.
	 * In the compact C-OS port, waiting on several synchronous script
	 * subresources delays the first useful paint.  Register the script as
	 * already handled, without creating a child fetch or pausing hubbub. */
	if (strstr(nsurl_access(joined), "https://www.wikipedia.org/") ==
		    nsurl_access(joined) &&
	    strstr(nsurl_access(joined), ".js") != NULL) {
		nscript = html_process_new_script(c, mimetype, HTML_SCRIPT_ASYNC);
		if (nscript == NULL) {
			nsurl_unref(joined);
			content_broadcast_error(&c->base, NSERROR_NOMEM, NULL);
			return DOM_HUBBUB_NOMEM;
		...
		nsurl_unref(joined);
		NSLOG(netsurf, INFO, "skipping optional Wikimedia script");
