...
box_construct_element(struct box_construct_ctx *ctx, bool *convert_children)
{
	...
	/* Replaced form controls choose their native inline-block box type in
	 * convert_special_elements().  CSS flex blockification must happen after
	 * that conversion as well: otherwise adjacent direct children such as
	 * input, input and button are placed in one anonymous inline flex item.
	 * This applies to any inline-block/inline-flex direct child, not to a
	 * particular document or control type. */
	if (box__containing_block_is_flex(&props)) {
		switch (box->type) {
		case BOX_INLINE_FLEX:
			box->type = BOX_FLEX;
			break;
