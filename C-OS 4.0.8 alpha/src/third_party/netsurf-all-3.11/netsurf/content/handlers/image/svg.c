...
#define BGR(c) ((c) == svgtiny_TRANSPARENT ? NS_TRANSPARENT :		\
	...
	for (i = 0; i != diagram->shape_count; i++) {
		if (diagram->shape[i].path) {
			/* `plot_style_t` carries operation types separately from colours.
			 * Initialising colours alone leaves stack garbage to decide whether a
			 * shape is filled/stroked, which caused valid SVGs to black-fill. */
			...
			pstyle.stroke_type = (diagram->shape[i].stroke == svgtiny_TRANSPARENT) ?
				PLOT_OP_TYPE_NONE : PLOT_OP_TYPE_SOLID;
			pstyle.fill_type = (diagram->shape[i].fill == svgtiny_TRANSPARENT) ?
				PLOT_OP_TYPE_NONE : PLOT_OP_TYPE_SOLID;
