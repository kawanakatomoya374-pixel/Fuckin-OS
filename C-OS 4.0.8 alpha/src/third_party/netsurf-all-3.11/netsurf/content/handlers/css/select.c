...
/* A converted NetSurf document is normally immutable, so libcss node data
 * lives until the DOM itself is destroyed.  The QuickJS bridge deliberately
 * keeps its libdom document alive and rebuilds only boxes after mutation; walk
 * the live tree and release that per-node state before creating a new selector
 * context. */
static void html_css_clear_node_data_subtree(dom_node *node)
{
	dom_node *child = NULL;
	dom_node *next = NULL;
	void *old_data = NULL;
	...
	if (node == NULL) return;
	if (dom_node_set_user_data(node, corestring_dom___ns_key_libcss_node_data,
			NULL, NULL, &old_data) == DOM_NO_ERR && old_data != NULL) {
		(void)css_libcss_node_data_handler(&selection_handler,
				CSS_NODE_DELETED, NULL, node, NULL, old_data);
	}
	...
	while (child != NULL) {
		...
		html_css_clear_node_data_subtree(child);
...
void html_css_clear_node_data(dom_node *root)
{
	html_css_clear_node_data_subtree(root);
}
