...
#ifndef COS_KERNEL
/* Tier B rendering (see PORTING_NOTES.md): needs a real plotter and
 * the knockout-rendering helpers (desktop/knockout.c, not part of
 * this build), neither of which exist yet. Not called by anything in
 * the current synchronous content-pipeline driver (cos_netsurf.c),
 * which doesn't draw. */
/* exported interface, documented in content/content.h */
bool
...
#ifndef COS_KERNEL
	/* content/textsearch.c (find-in-page) isn't part of this build -
	 * Tier B, needs real rendering to be meaningful (see
	 * PORTING_NOTES.md). c->textsearch.context is never set to
	 * non-NULL anywhere reachable without it either, so this is
