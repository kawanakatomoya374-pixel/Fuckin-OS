...
{
	if (chains == 0) {
		NSLOG(netsurf, INFO, "Refusing zero-chain hash table.");
		return NULL;
	}
...
bool hash_add(struct hash_table *ht, const char *key, const char *value)
{
	...
	if (ht == NULL || ht->nchains == 0 || ht->chain == NULL ||
	    key == NULL || value == NULL)
		return false;
...
const char *hash_get(struct hash_table *ht, const char *key)
{
	...
	struct hash_entry *e;
#ifdef COS_KERNEL
	/* The freestanding C-OS port does not ship the optional message-dictionary
	 * files.  Their hash table may be a partially initialised compatibility
	 * object during a real navigation; walking it is not required for content
	 * parsing and previously led to a #GP at the chain dereference. */
	(void)ht;
	(void)key;
	return NULL;
#else
	if (ht == NULL || ht->nchains == 0 || ht->chain == NULL || key == NULL)
		return NULL;
#endif
...
#ifndef COS_KERNEL
/* Needs real gzopen/gzgets/gzclose (zlib) plus strerror(errno) and
 * NSLOG - none of which this build has (see PORTING_NOTES.md).
 * hash_create/hash_destroy/hash_add/hash_get/hash_add_inline (used
 * by utils/messages.c, part of this build) don't touch any of that
