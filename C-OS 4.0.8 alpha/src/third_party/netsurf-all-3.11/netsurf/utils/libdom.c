...
#ifndef COS_KERNEL
/* The rest of this file (the debug structure-dump helpers, and
 * libdom_parse_file() below) is built around real hosted FILE*
 * (fopen/fread/feof/fclose/fprintf), which this freestanding kernel
 * build doesn't have. It's excluded here rather than given a fake
 * FILE* implementation. Nothing is actually lost for real page
 * loading: libdom_parse_file() is a thin convenience wrapper around
 * dom_hubbub_parser_create()/dom_hubbub_parser_parse_chunk()/
 * dom_hubbub_parser_completed() (see libdom/bindings/hubbub/parser.c,
 * already part of this build) that supplies chunks by reading a local
 * file - a future C-OS content handler can call that same chunk API
 * directly with data from kernel/api/net_api.c's http_fetch() or the
 * filesystem API instead of going through fopen() at all. */
