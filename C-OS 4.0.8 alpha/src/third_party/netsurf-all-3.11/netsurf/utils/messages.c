 ...
 */
#ifndef COS_KERNEL
/* Needs hash_add_file (gzip+file I/O, excluded from this build -
 * see utils/hashtable.c's COS_KERNEL guards). No localisation file
 * to load anyway (this kernel boots from an ISO, not a desktop
 * install with a resource tree). messages_get_errorcode() and
 * messages_get() below are untouched and remain usable. */
...
#ifndef COS_KERNEL
/* Calls messages_load_ctx (guarded above). No localization file
 * to load in this kernel build anyway. */
