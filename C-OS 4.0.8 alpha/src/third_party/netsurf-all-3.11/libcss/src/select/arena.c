...
css_error css__arena_intern_style(struct css_computed_style **style)
{
	...
	/* The C-OS freestanding allocator has no process-isolated lifetime for
	 * libcss's global cross-document arena.  Reusing stale linked-list entries
	 * after a browser navigation can yield non-canonical pointers.  Keep each
	 * style private, while retaining the usual initial ownership reference so
	 * css__computed_style_ref()/destroy() keep shared box styles valid. */
	(*style)->count = 1;
	return CSS_OK;
#else
