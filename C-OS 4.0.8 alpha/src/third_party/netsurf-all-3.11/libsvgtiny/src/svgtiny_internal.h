...
void svgtiny_transform_path(float *p, unsigned int n,
		struct svgtiny_parse_state *state);
#ifdef COS_KERNEL
/* The kernel has no hosted libc even when a build environment exposes
 * _GNU_SOURCE. Always use libsvgtiny's local allocator-backed implementation. */
#undef HAVE_STRNDUP
char *svgtiny_strndup(const char *s, size_t n);
#define strndup svgtiny_strndup
#elif (defined(_GNU_SOURCE) && !defined(__APPLE__) || defined(__amigaos4__) || defined(__HAIKU__) || (defined(_POSIX_C_SOURCE) && ((_POSIX_C_SOURCE - 0) >= 200809L)))
#define HAVE_STRNDUP
#else
#undef HAVE_STRNDUP
char *svgtiny_strndup(const char *s, size_t n);
