...
#include <dom/dom.h>
/* The standalone libsvgtiny release normally uses libdom's XML binding, which
 * requires an external Expat or libxml2 port. C-OS deliberately has neither
 * hosted dependency. Its already-linked Hubbub/libdom binding still constructs
 * the same DOM node/attribute API required below, so use it only for the
 * freestanding parser path. Hosted NetSurf builds retain upstream XML parsing. */
#ifdef COS_KERNEL
		...
		int viewport_width, int viewport_height)
{
	...
	parser_params.enc = "UTF-8";
	parser_params.fix_enc = true;
	parser_params.enable_script = false;
	parser_params.msg = ignore_msg;
	parser_params.ctx = NULL;
	err = dom_hubbub_parser_create(&parser_params, &parser, &document);
	if (err != DOM_HUBBUB_OK || parser == NULL)
		return svgtiny_LIBDOM_ERROR;

	err = dom_hubbub_parser_parse_chunk(parser, (const uint8_t *)buffer, size);
	...
	/* The page parser has yielded a standalone libdom document. Destroy the
	 * parser before traversing it, exactly as the upstream XML path does. */
	dom_hubbub_parser_destroy(parser);
#else
	...
	dom_xml_parser_destroy(parser);
		...
		/* The XML binding exposes <svg> as the document element. The compact
		 * C-OS path uses the already linked Hubbub binding, which normalises a
		 * standalone SVG into html/head/body/svg. Locate the first SVG element
		 * in that DOM rather than assuming the document root is SVG. */
#ifdef COS_KERNEL
		{
			dom_string *svg_search_name = NULL;
			dom_nodelist *svg_nodes = NULL;
			uint32_t svg_count = 0;
			exc = dom_string_create_interned((const uint8_t *)"svg", 3,
							 &svg_search_name);
			if (exc != DOM_NO_ERR) {
				dom_node_unref(document);
				return svgtiny_LIBDOM_ERROR;
			...
			exc = dom_document_get_elements_by_tag_name(document,
					svg_search_name, &svg_nodes);
			dom_string_unref(svg_search_name);
			if (exc != DOM_NO_ERR || svg_nodes == NULL ||
				dom_nodelist_get_length(svg_nodes, &svg_count) != DOM_NO_ERR ||
				svg_count == 0) {
				if (svg_nodes != NULL) dom_nodelist_unref(svg_nodes);
				dom_node_unref(document);
				return exc == DOM_NO_ERR ? svgtiny_SVG_ERROR : svgtiny_LIBDOM_ERROR;
			}
			exc = dom_nodelist_item(svg_nodes, 0, (dom_node **)&svg);
			dom_nodelist_unref(svg_nodes);
			if (exc != DOM_NO_ERR || svg == NULL) {
				dom_node_unref(document);
				return svgtiny_LIBDOM_ERROR;
			}
		}
