...
typedef struct {
    uint64_t base_low;      /* bits 0-15 of handler address */
    uint64_t selector;      /* code segment selector */
    uint8_t  ist;           /* interrupt stack table (0 = none) */
    uint8_t  flags;         /* type and attribute flags */
    uint64_t base_mid;      /* bits 16-31 of handler address */
    uint64_t base_high;     /* bits 32-63 of 64-bit handler address - fully 64-bit */
